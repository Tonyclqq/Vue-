...
```
//如果使用Vue.component 定义全局组建的时候，组件名称使用了 驼峰命名，则在引用组建的时候，需要把大学的驼峰改为小写的字母，同时，两个单词之间，使用-链接；
...
Vue.component('account', {
    ...
    //组建的data数据，使用方式，和实例中的data使用方式完全一样。
```
...
1. props中的数据都是通过父组件传递给子组件的。

   data的数据不是通过父组件传递过来的，是自己私有的。并且子组件的data数据，并不是通过父组件传递过来的，而是子组件自身私有的，比如：子组件通过Ajax，请求回来的数据，都可以放到data身上

2. data上的数据是可读可写的， props中的数据都是只读，无法重新赋值。



### 父组件向子组件 传递方法，使用的是事件绑定机制；v-on，当我们自定义了一个事件属性之后，那么，子组件就能够通过某些方式，来调用传递进去的这个方法了。
